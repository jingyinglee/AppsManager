...
//        }
    ...
    @Thunk public void startAppShortcutOrInfoActivity(View v) {
        Object tag = v.getTag();
    ...
    //启动某应用后，一定会最终运行到这里。这里承载相关业务需求 20171219 abylee
            ...
            switch (item.itemType) {
                ...
                case LauncherSettings.Favorites.ITEM_TYPE_PRESET_SHORTCUT:
                    ...
                    if (item.itemType == LauncherSettings.Favorites.ITEM_TYPE_PRESET_SHORTCUT){
                        Log.e(TAG, "bindItems "+info.title.toString());
//                        String url = info.downloadUrl;
//                        String path = TasksManager.getImpl().createPath(info.title.toString());
//                        TasksManager.getImpl().addTask(url, path);
//
//                        final int id = FileDownloadUtils.generateId(info.downloadUrl, TasksManager.getImpl().createPath(info.title.toString()));
